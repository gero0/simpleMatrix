...
namespace mx{
    ...
    public:
        ...
        Matrix(Dimensions d, std::initializer_list<T> l) 
        : dim(d), elements(l) {
            if ( (dim.rows*dim.collumns) != l.size() )
                throw std::invalid_argument("Matrix: Make sure that row_count * collumn_count = list.lenght()");
        ...
        bool isSquare() const{
            return (dim.collumns == dim.rows);
        }

        T& at(uint32_t row, uint32_t collumn){
            if(row > dim.rows || collumn > dim.collumns || row < 1 || collumn < 1){
        ...
        Matrix subMatrix(Dimensions start, Dimensions range){
            if( (start.collumns + range.collumns) > dim.collumns || (start.rows + range.rows) > dim.rows || start.collumns < 1 || start.rows < 1)
        ...
        Matrix minor(Dimensions number){
            if( number.collumns > dim.collumns || number.rows > dim.rows || number.collumns < 1 || number.rows < 1)
        ...
        Matrix invert(){
            ...
            if (determinant == 0)
                throw std::invalid_argument("Cannot invert matrix, determinant is equal to zero");

            Matrix inverted {dim};

            for(uint32_t i=1; i<=dim.rows; i++){
                for(uint32_t j=1; j<=dim.collumns; j++){
                    inverted.at(j,i) = minor( {i,j} ).det() * (1/determinant) * pow(-1, i+j) ;
                }
            }
    ...
    Matrix<T> operator*( Matrix<T> &a, Matrix<T> &b) {
        if(a.getDimensions().collumns == b.getDimensions().rows){
            ...
            Matrix<T> mx { Dimensions {a.getDimensions().rows, b.getDimensions().collumns} };
            for(int i=1; i<=d.rows; i++){
                for(int j=1; j<=d.collumns; j++){
                    mx.at(i,j) = 0;
